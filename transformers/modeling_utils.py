    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                # Sample
                next_token_softmax = F.softmax(next_token_logits, dim=-1)
                next_token = torch.multinomial(next_token_softmax, num_samples=1).squeeze(1)
            else:
                # Greedy decoding
...
def top_k_top_p_filtering(logits, top_k=0, top_p=1.0, filter_value=-float("Inf"), min_tokens_to_keep=1):
    """ Filter a distribution of logits using top-k and/or nucleus (top-p) filtering
        ...
        Sample for how this code works.
        logits = tensor([[-0.9723, -0.1009,  0.7504], [-0.2527,  0.4749, -0.9140]])
        sorted_logits = tensor([[ 0.7504, -0.1009, -0.9723], [ 0.4749, -0.2527, -0.9140]])
        sorted_indices = tensor([[2, 1, 0], [1, 0, 2]])
        sorted_logits_softmax = tensor([[0.6229, 0.2659, 0.1112], [0.5772, 0.2788, 0.1439]])
        cumulative_probs = tensor([[0.6229, 0.8888, 1.0000], [0.5772, 0.8561, 1.0000]]
        sorted_indices_to_remove = tensor([[0, 0, 1], [0, 0, 1]], dtype=torch.uint8)
        indices_to_remove = tensor([[1, 0, 0], [0, 1, 0]], dtype=torch.uint8)
        final logits = tensor([[-inf, -0.7148, -0.6536], [-0.8201,    -inf, -0.1985]])

        # Note the scatter function is *really slow*
        torch.max operation takes about 25% of all generation time.
        torch.scatter operation takes about 50% of all generation time.
        There seems to be no easy way to improve these two functions.
        There seems to be no replacement for scatter.
