...
class ContextGenerator(object):
    ...
    def __init__(self, model_path, add_reverse=True):
    ...
    def generate(self, qa_file_path: str, outpath: str):
    ...
    def generate_context_batched(self, node_text: str, para_text: str,
        ...
        queries = self.create_queries(
            node_text, para_text, formatter=SourceTextFormatter.natural_question_formatter)
        context_str = ""
    ...
    def create_queries(self, node_text: str, para_text: str, formatter) -> List[str]:

        context = ""
        res = []
        for rel in self.relations:
            res.append(formatter(
                relation=rel, path_length=Rels.special_tokens['1'], src=node_text, para=para_text))
            # res.append(
            #    f"{rel} {Rels.special_tokens['1']} {Rels.special_tokens['node_sep']} {node_text} {Rels.special_tokens['para_sep']} {para_text}")
        return res

    def batch_generate(self, input_sentences: List[str], beam_search=False,
                       clean_output_flag=True) -> str:
        encoded_prompts = []
        prompt_lengths = []
...
if __name__ == '__main__':
    ...
    context_generator.generate(
        qa_file_path=args["--qa-file-path"], outpath=args["--outpath"])
