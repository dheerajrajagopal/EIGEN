        ...
        if os.path.exists(cached_features_file) and os.path.exists(cached_metadata_file):
            ...
            with open(file_path, encoding="utf-8") as f:
                for line in f:
                    ...
                    token_ids, token_labels, one_metadata = TWDataset.read_line(input_json=input_json,
                                                                                tokenizer=tokenizer,
    ...
    def read_line(input_json, tokenizer, block_size, skip_answer, stop_token='<|endoftext|>'):
        ...
        if not skip_answer:
            if 'answer' not in input_json or input_json['answer'] == '' :
                ...
                tokenized_answer = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(input_json['answer'])) if not skip_answer else []
        else:
            tokenized_answer = []

        metadata['id'] = input_json['id']
        TWDataset._truncate_seq_pair(tokenized_question, tokenized_answer, max_length=block_size - 1)
...
def load_and_cache_examples(tokenizer, output_dir:str, file_path:str, block_size:int):
    return TWDataset(tokenizer=tokenizer,
                     file_path=file_path,
                     block_size=block_size,
